  ...
  <div class="p-6 bg-gray-100 min-h-screen">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6 max-w-lg">
      <form @submit.prevent="addSupplier" class="space-y-6">
        ...
        <button type="submit" class="w-full bg-indigo-600 text-white py-2 px-4 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150 ease-in-out flex items-center justify-center">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="supplier in suppliers" :key="supplier.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap flex space-x-2">
                <button @click="openEditModal(supplier)" class="text-indigo-600 hover:text-indigo-900"><i class="fas fa-edit"></i></button>
                <button @click="deleteSupplier(supplier.id)" class="text-red-600 hover:text-red-900"><i class="fas fa-trash-alt"></i></button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <form @submit.prevent="updateSupplier" class="space-y-4">
          <div>
            ...
            <div class="mt-1 relative">
              <input v-model="editSupplier.name" id="edit-name" type="text" required class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white shadow-sm transition duration-150 ease-in-out" placeholder="Enter supplier name">
              <span class="absolute left-3 top-2 text-gray-400"><i class="fas fa-user"></i></span>
            </div>
          </div>
          <div>
            ...
            <div class="mt-1 relative">
              <input v-model="editSupplier.address" id="edit-address" type="text" required class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white shadow-sm transition duration-150 ease-in-out" placeholder="Enter address">
              <span class="absolute left-3 top-2 text-gray-400"><i class="fas fa-map-marker-alt"></i></span>
            </div>
          </div>
          <div>
            ...
            <div class="mt-1 relative">
              <input v-model="editSupplier.email" id="edit-email" type="email" required class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white shadow-sm transition duration-150 ease-in-out" placeholder="Enter email">
              <span class="absolute left-3 top-2 text-gray-400"><i class="fas fa-envelope"></i></span>
            </div>
          </div>
